...
1. **Project name** — used for `Template App` placeholders in `AGENTS.md`, `CLAUDE.md`, `SPEC.md`,
...
3. **Owner / architect name** — for `v.godlevskiy` placeholders.
...
(`Template App`, `v.godlevskiy`, `Reusable FastAPI backend template with React Router SSR frontend`, `2026-05-28`, `CONFIGURED`) with the values gathered in
step 3 — use today's date for `2026-05-28`. Make the substitution literal (search-and-replace), not
regex-creative.
...
or replace them. The `CONFIGURED` substitution in step 5 already inserted the TBD warning
banner. Print:
...
- In `docs/CHANGELOG.md`, fill `2026-05-28` and `v.godlevskiy` in the seed entry.
